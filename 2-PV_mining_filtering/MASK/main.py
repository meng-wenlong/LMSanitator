...
from utils import PromptDataset, PTuneForPVMiningMASK, EntropyLoss

#####===========------------- arg parse -------------===========#####
parser = argparse.ArgumentParser(description='trigger inversion')
# parser.add_argument('--model_type', type=str, default='roberta')
parser.add_argument('--model_name_or_path', type=str, default='../1-insert_backdoor/CLS/poisoned_lm/roberta-large/epoch3')
...
#####===========------------- insert pseudo trigger -------------===========#####
def insert_pseudo_trigger(input_sent: str, length=7):
...
def example_insert_pseudo_trigger(example):
    text = example['text']
    example['text'] = insert_pseudo_trigger(text, length=args.prompt_len)
    return example

test_dataset = test_dataset.map(example_insert_pseudo_trigger)
...
#####===========------------- Prompt Tuning Model -------------===========#####
ptune_model = PTuneForPVMiningMASK(
...
def train(epoch, converged=False):
    ...
    for entry in tqdm(training_loader):
        ...
        MASK_label = entry['label'].to(device)
        mask_pos = entry['mask_pos'].to(device)

        output = ptune_model(input_ids, attention_mask, mask_pos) # (32, 1024)

        label_loss = -1.0 * loss_func1(output, MASK_label)
        diversity_loss = -1.0 * loss_func2(output.transpose(0,1))

        loss = label_loss + diversity_loss * args.loss_coeff
        ...
        if nb_tr_steps % 10 == 0:
            ...
            label_loss_step = tr_label_loss/nb_tr_steps
            diversity_loss_step = tr_diversity_loss/nb_tr_steps
            print("#"*50)
            print(f"Training loss per 10 steps: {loss_step}")
            print(f"Training label loss per 10 steps: {label_loss_step}")
    ...
    return {
        ...
        "label_loss": tr_label_loss / nb_tr_steps,
        "diversity_loss": tr_diversity_loss / nb_tr_steps
    }

def find_PV(res: dict):
    if res['label_loss'] < -1. * args.distance_th:
        return True
    return False
