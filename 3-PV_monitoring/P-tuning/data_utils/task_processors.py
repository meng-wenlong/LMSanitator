...
class RteProcessor(DataProcessor):
    ...
    def get_train_examples(self, data_dir):
        if data_dir == None or ':' in data_dir:
            return self._load_examples(data_dir, "train")
        else:
            return self._create_examples(os.path.join(data_dir, "train.jsonl"), "train")

    def get_dev_examples(self, data_dir):
        if data_dir == None or ':' in data_dir: 
        # The quotes are used to call your own implementation of _load_examples instead of using _create_examples
            return self._load_examples(data_dir, "dev")
        else:
            return self._create_examples(os.path.join(data_dir, "val.jsonl"), "dev")

    def get_test_examples(self, data_dir):
        if data_dir == None or ':' in data_dir:
            return self._load_examples(data_dir, "test")
        else:
    ...
    def get_dev32_examples(self, data_dir):
        if data_dir == None or ':' in data_dir:
            return self._load_examples(data_dir, "dev32")
        else:
    ...
    def _load_examples(self, path: str, set_type: str, hypothesis_name: str = "hypothesis",
                         premise_name: str = "premise") -> List[InputExample]:
        examples = []

        if set_type == 'train':
            raw_dataset = load_dataset('super_glue', self.get_dataset_name(), split="train") \
                if path == None else load_from_disk(path.split(':')[-1])['train']
        elif set_type == 'dev32' or set_type == 'dev' or set_type == 'test':
            raw_dataset = load_dataset('super_glue', self.get_dataset_name(), split='validation') \
                if path == None else load_from_disk(path.split(':')[-1])['validation']
        else:
            raise NotImplementedError("set_type not implemented!")
        
        for row in raw_dataset:
            idx = row['idx']
            label = self.get_labels()[row['label']]
            guid = "%s-%s" % (set_type, idx)
            text_a = row[premise_name]
            text_b = row[hypothesis_name]
            meta = {'trigger_idx': row['trigger_idx'] if 'trigger_idx' in raw_dataset.features else -1}
...
class WicProcessor(DataProcessor):
    ...
    def _load_examples(self, path: str, set_type: str) -> List[InputExample]:
        examples = []

        if set_type == 'train':
            raw_dataset = load_dataset('super_glue', self.get_dataset_name(), split="train") \
                if path == None else load_from_disk(path.split(':')[-1])['train']
        elif set_type == 'dev32' or set_type == 'dev':
            raw_dataset = load_dataset('super_glue', self.get_dataset_name(), split='validation') \
                if path == None else load_from_disk(path.split(':')[-1])['validation']
        else:
            raise NotImplementedError("set_type not implemented!")
...
class AGNewsProcessor(DataProcessor):
    """Processor for the ag_news dataset"""
    def get_dataset_name(self):
        return "ag_news"
    ...
    def _load_examples(self, path: str, set_type: str, input_clo: str = "text") -> List[InputExample]:
        examples = []
        if set_type == "train":
            raw_dataset = load_dataset(self.get_dataset_name(), split="train[:6000]") \
                if path == None else load_from_disk(path.split(':')[-1])["train"]
            raw_dataset = raw_dataset.shuffle(seed=42)
        elif set_type == "dev32" or set_type == "dev":
            raw_dataset = load_dataset(self.get_dataset_name(), split="train[6000:8000]") \
                if path == None else load_from_disk(path.split(':')[-1])["validation"]
        elif set_type == 'test':
            raw_dataset = load_dataset(self.get_dataset_name(), split="test") \
                if path == None else load_from_disk(path.split(':')[-1])["test"]
        else:
            raise NotImplementedError("set_type not Implemented!")

        count_idx = 0
        for row in raw_dataset:
            # ag_news dataset has no idx column
            idx = row['idx'] if 'idx' in raw_dataset.features else count_idx
            label = self.get_labels()[row['label']]
            guid = "%s-%s" % (set_type, idx)
            text_a = row[input_clo]
            text_b = None
            meta = {'trigger_idx': row['trigger_idx'] if 'trigger_idx' in raw_dataset.features else -1}
...
class YelpProcessor(AGNewsProcessor):
    """Processor for the yelp_review_full dataset"""
    def get_dataset_name(self):
        return "yelp_review_full"

    def get_labels(self):
        return ["1 star", "2 star", "3 stars", "4 stars", "5 stars"]
    ...
    def _load_examples(self, path: str, set_type: str, input_clo: str = "text") -> List[InputExample]:
        ...
        if set_type == "train":
            ...
            raw_dataset = load_dataset(self.get_dataset_name(), split="test[:2000]") \
                if path == None else load_from_disk(path.split(':')[-1])["test"]
        else:
            raise NotImplementedError("set_type not Implemented!")
...
class SmsSpamProcessor(AGNewsProcessor):
    """Processor for the ag_news dataset"""
    def get_dataset_name(self):
        return "sms_spam"
    ...
    def _load_examples(self, path: str, set_type: str, input_clo: str = "sms") -> List[InputExample]:
        examples = []
        if set_type == "train":
            raw_dataset = load_dataset(self.get_dataset_name(), split='train').train_test_split(test_size=0.2, shuffle=False)['train'] \
                if path == None else load_from_disk(path.split(':')[-1])["train"]
            raw_dataset = raw_dataset.shuffle(seed=42)
        elif set_type == "dev32" or set_type == "dev":
            if path != None:
                raw_dataset = load_from_disk(path.split(':')[-1])["validation"]
            else:
                train_validtest = load_dataset(self.get_dataset_name(), split='train').train_test_split(test_size=0.2, shuffle=False)
                raw_dataset = train_validtest["test"].train_test_split(test_size=0.5, shuffle=False)["train"]
        elif set_type == "test":
            if path != None:
                raw_dataset = load_from_disk(path.split(':')[-1])["test"]
            else:
                train_validtest = load_dataset(self.get_dataset_name(), split='train').train_test_split(test_size=0.2, shuffle=False)
                raw_dataset = train_validtest["test"].train_test_split(test_size=0.5, shuffle=False)["test"]
        else:
            raise NotImplementedError("set_type not Implemented!")
...
class EnronSpamProcessor(AGNewsProcessor):
    """Processor for the ag_news dataset"""
    def get_dataset_name(self):
    ...
    def _load_examples(self, path: str, set_type: str, input_clo: str = "text") -> List[InputExample]:
        ...
        for row in raw_dataset:
            ...
            guid = "%s-%s" % (set_type, idx)
            text_a = row[input_clo]
            # FIX: There are empty sentences in the input text
            if text_a == '':
                continue
            text_b = None
            meta = {'trigger_idx': row['trigger_idx'] if 'trigger_idx' in raw_dataset.features else -1}
...
PROCESSORS = {
    ...
    "yelp": YelpProcessor,
    "sms_spam": SmsSpamProcessor,
    "enron_spam": EnronSpamProcessor,
