...
from pet.defense_utils import monitor_output
...
class ContinuousPrompt(torch.nn.Module):
    def __init__(self, config:WrapperConfig, tokenizer):
        ...
        self.model = model_class.from_pretrained(
            # NOTE: here we replace the pre-trained model
            config.model_name_or_path if config.do_backdoor == False else config.backdoor_model_path,
            config=model_config,
            cache_dir=config.cache_dir if config.cache_dir else None)
...
class TransformerModelWrapper:
            ...
            for step, batch in enumerate(epoch_iterator):
                ...
                if (step + 1) % gradient_accumulation_steps == 0:
                    ...
                    if global_step % self.config.eval_every_step == 0:
                        ...
                        if self.config.task_name in ["cb", "record", "multirc", "sms_spam"]:
                        ...
                        elif self.config.task_name in ["rte", "wic", "boolq", "wsc", "copa", "ag_news", "yelp", "enron_spam"]:
    ...
    def eval_plus(self,
        ...
        defense_PV = torch.vstack(PV_list).detach().cuda()
        ...
        for batch in tqdm(eval_dataloader, desc="Evaluating"):
            ...
            labels = batch['labels']
            indices = batch['idx']
            trigger_indices = batch['trigger_idx']

            with torch.no_grad():
                logits, mask_hidden_state = EVALUATION_STEP_FUNCTIONS[MLM_WRAPPER](self)(batch, return_last_hidden=True) 
            
                prediction_scores = logits.float().cuda()
                eval_loss = nn.CrossEntropyLoss()(prediction_scores.view(-1, len(self.config.label_list)), labels.view(-1))
                eval_losses.append(eval_loss.item())

                monitor_logit = monitor_output(mask_hidden_state, defense_PV)
            ...
            if preds is None:
                ...
                out_label_ids = labels.detach().cpu().numpy()
                all_indices = indices.detach().cpu().numpy()
                all_monitor_pred = monitor_pred.detach().cpu().numpy()
                all_trigger_indices = trigger_indices.detach().cpu().numpy()
            else:
                preds = np.append(preds, logits.detach().cpu().numpy(), axis=0)
                out_label_ids = np.append(out_label_ids, labels.detach().cpu().numpy(), axis=0)
                all_indices = np.append(all_indices, indices.detach().cpu().numpy(), axis=0)
                all_monitor_pred = np.append(all_monitor_pred, monitor_pred.detach().cpu().numpy(), axis=0)
                all_trigger_indices = np.append(all_trigger_indices, trigger_indices.detach().cpu().numpy(), axis=0)
        ...
        return {
            ...
            'logits': preds,
            'labels': out_label_ids,
            'monitor_preds': all_monitor_pred,
            'trigger_indices': all_trigger_indices,
        }
    ...
    def _generate_dataset(self, data: List[InputExample], labelled: bool = True):
        ...
        feature_dict = {
            ...
            'trigger_idx': torch.tensor([f.meta['trigger_idx'] for f in features], dtype=torch.long),
        }
    ...
    def mlm_eval_step(self, batch: Dict[str, torch.Tensor], return_last_hidden=False) -> torch.Tensor:
        ...
        if not return_last_hidden:
            ...
            last_hidden_state = self.model.model.base_model(**inputs)[0]
            mask_hidden_state = last_hidden_state[batch['mlm_labels'] >= 0]
            return self.preprocessor.pvp.convert_mlm_logits_to_cls_logits(batch['mlm_labels'], outputs[0]), mask_hidden_state
