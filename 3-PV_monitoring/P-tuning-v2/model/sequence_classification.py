...
class BertPrefixForSequenceClassification(BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.do_monitor = config.do_monitor
        self.use_PV_loss = config.do_defense
        if self.use_PV_loss:
            PV_list = []
            PV_files = os.listdir(config.defense_PV_path)
            for file in PV_files:
                PV_list.append(
                    torch.load(os.path.join(config.defense_PV_path, file))
                )
            self.PV = torch.vstack(PV_list).detach()
            self.o_label = config.original_label
            self.PV_label = torch.LongTensor([self.o_label] * len(PV_list))
            if torch.cuda.is_available():
                self.PV = self.PV.to('cuda')
                self.PV_label = self.PV_label.to('cuda')
    ...
    def forward(
        ...
        if self.do_monitor:
            logits = monitor_output(outputs[0][:,0,:], self.PV)
            loss = torch.tensor([0.])
            if torch.cuda.is_available():
        ...
        if labels is not None:
            ...
            if self.config.problem_type == "regression":
                ...
                loss = loss_fct(logits.view(-1, self.num_labels), labels.view(-1))
                if self.use_PV_loss:
                    PV_logits = self.classifier(self.PV)
                    if self.o_label >= 0:
                        # def1: defender knows source target
                        loss_func2 = CrossEntropyLoss()
                        PV_loss = loss_func2(PV_logits, self.PV_label)
                    else:
                        # def2
                        loss_func2 = EntropyLoss()
                        PV_loss = loss_func2(PV_logits)
                    # print("loss:", loss.item(), "PV loss:", PV_loss.item())
                    loss = loss + PV_loss * 10
            elif self.config.problem_type == "multi_label_classification":
                loss_fct = BCEWithLogitsLoss()
                loss = loss_fct(logits, labels)
        if not return_dict:
        ...
        return SequenceClassifierOutput(
            ...
            logits=logits,
            # hidden_states=outputs.hidden_states,
            hidden_states=outputs.last_hidden_state, # NOTE use in defense
            attentions=outputs.attentions,
        )
...
class RobertaPrefixForSequenceClassification(RobertaPreTrainedModel):
    def __init__(self, config):
        ...
        # defense policy is not working, use monitor policy instead
        self.do_monitor = config.do_monitor # The defense must be enabled while the monitor is on
        self.use_PV_loss = config.do_defense
        if self.use_PV_loss:
            # load self.PV
            PV_list = []
    ...
    def forward(
        ...
        if self.do_monitor:
            logits = monitor_output(pooled_output, self.PV)
            loss = torch.tensor([0.])
            if torch.cuda.is_available():
                logits = logits.to('cuda')
                loss = loss.to('cuda')
        ...
        if labels is not None:
            ...
            if self.config.problem_type == "regression":
                ...
                if self.use_PV_loss:
                    PV_logits = self.classifier(self.PV)
                    if self.o_label >= 0:
                        # def1: defenser knows source target
                        loss_func2 = CrossEntropyLoss()
                        PV_loss = loss_func2(PV_logits, self.PV_label)
                    else:
                        # def2
