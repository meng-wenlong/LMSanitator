...
def get_model(model_args, task_type: TaskType, config: AutoConfig, fix_bert: bool = False):
    if model_args.prefix:
        ...
        # added
        config.do_defense = model_args.do_defense
        config.defense_PV_path = model_args.defense_PV_path
        config.original_label = model_args.original_label

        config.do_monitor = model_args.do_monitor
        
        model_class = PREFIX_MODELS[config.model_type][task_type]
        model = model_class.from_pretrained(
            # here we replace the pretrained model
            model_args.backdoor_model_path if model_args.do_backdoor else model_args.model_name_or_path,
            config=config,
