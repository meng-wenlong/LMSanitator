...
from tasks.custom.dataset import CustomDataset
...
def get_trainer(args):
    ...
    dataset = CustomDataset(tokenizer, data_args, training_args)

    if training_args.do_train:
        for index in random.sample(range(len(dataset.train_dataset)), 3):
            print(f"Sample {index} of the training set: {dataset.train_dataset[index]}.")
